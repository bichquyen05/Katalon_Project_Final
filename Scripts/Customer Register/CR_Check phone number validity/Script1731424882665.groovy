...
boolean isLength = phoneNumber.length() >= 10 && phoneNumber.length() <= 15
boolean isDigit = phoneNumber.isNumber()
boolean hasWhiteSpace = phoneNumber.contains(" ")
...
if(messagePresent) {
	...
	if(!isLength || !isDigit || hasWhiteSpace) {
		WebUI.verifyMatch(message, 'Phone numbers must contain only numbers, be 10 - 15 characters long and contain no spaces.', false)
	}
